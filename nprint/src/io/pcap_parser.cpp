...
// this is the main part of the program, loops for every packet and calls a handler
void PCAPParser::process_file() {
    ...
    // main loop -> executes packet_handler for each packet
    pcap_loop(f, 0, packet_handler, (u_char *)this);
    pcap_close(f);
}

// This is the part of the program that takes care of processing each packet
void PCAPParser::packet_handler(u_char *user_data,
                                const struct pcap_pkthdr *pkthdr,
                                const u_char *packet) {

    // packet represents the packet in memory
    PCAPParser *pcp;
    SuperPacket *sp;
    int64_t rts;
    ...
    // in here we have a reference to the original packet
    // since PCAPparser does not implement it's own, this is the default in file_parser.ccp:44
    ...
    // here we write out the packet to standard output 
    pcp->write_output(sp);
}
