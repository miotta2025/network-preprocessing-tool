...
// this function is called for each packet to create a SuperPacket object that can be 
// printed to stdouptut 
SuperPacket *FileParser::process_packet(void *pkt) {
    ...
    to_fill.clear();
    // This object is the packet processed at each line
...
void FileParser::write_output(SuperPacket *sp) {
    // The bitstring is the output, basically a vector of 0,1,-1
    sp->get_bitstring(&config, bitstring_vec);
    // The definition is found in file_writer.cpp:131
    fw->write_bitstring_line(custom_output, bitstring_vec); // line that takes care of writing to output in the desired format
    bitstring_vec.clear();
    custom_output.clear();
